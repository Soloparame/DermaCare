    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="mb-6 flex items-center gap-4">
          ...
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Dr. {profile.fullName}</h3>
            <span className="rounded-full bg-teal-100 px-2 py-0.5 text-xs font-medium capitalize text-teal-700">
              {profile.specialization ?? "Dermatology"}
            </span>
          </div>
        </div>
        <dl className="grid gap-4 sm:grid-cols-2">
          ...
          {profile.consultationFee != null && (
            <div>
              ...
              <dd className="mt-0.5 text-slate-900">${Number(profile.consultationFee).toFixed(2)}</dd>
            </div>
          )}
          {profile.bio && (
            <div className="sm:col-span-2">
              <dt className="text-xs font-medium uppercase tracking-wide text-slate-500">Bio</dt>
              <dd className="mt-0.5 whitespace-pre-wrap text-slate-900">{profile.bio}</dd>
            </div>
          )}
        </dl>
      </div>
