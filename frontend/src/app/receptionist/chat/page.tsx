...
export default function ReceptionistChatPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await fetch<Appointment[]>("/receptionist/appointments");
  ...
  useEffect(() => {
    ...
    if (!token) return;
    const base = (typeof process !== "undefined" && process.env?.NEXT_PUBLIC_API_URL) || "http://localhost:4000/api";
    ...
    <div className="flex h-[calc(100vh-10rem)] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl shadow-slate-200/40">
      ...
      <div className="w-80 flex-shrink-0 border-r border-slate-200 bg-[#F8FAFC] flex flex-col relative z-20">
        <div className="p-5 border-b border-slate-200 bg-white shadow-sm">
          <h2 className="text-xl font-extrabold text-slate-800 tracking-tight">Support Chats</h2>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {appointments.map((a) => (
            ...
            >
              ...
              <div className="relative h-12 w-12 flex-shrink-0 rounded-full bg-gradient-to-br from-blue-400 to-indigo-500 flex items-center justify-center text-white font-bold text-lg shadow-sm">
              ...
              <div className="flex-1 overflow-hidden flex flex-col justify-center">
                <div className="flex justify-between items-center mb-1">
                  <h3 className="font-semibold text-slate-900 truncate">P. {a.patientName}</h3>
                  <span className="text-[10px] font-medium text-slate-400">{a.date}</span>
                </div>
                <p className="text-xs text-slate-500 truncate">Dr. {a.doctorName}</p>
              </div>
            </button>
          ))}
          {appointments.length === 0 && !error && (
            <div className="p-8 text-center text-slate-400 flex flex-col items-center mt-10">
              ...
              <p className="text-sm font-medium">No active support chats.</p>
            </div>
          )}
        </div>
      </div>
      ...
      {selectedAppointment ? (
        <div className="flex-1 flex flex-col bg-[#F0F2F5] relative">
          ...
          <div className="flex items-center justify-between px-6 py-4 bg-white/90 backdrop-blur-md border-b border-slate-200 z-10 shadow-sm">
            <div className="flex items-center gap-4">
              <div className="h-11 w-11 flex-shrink-0 rounded-full bg-gradient-to-br from-blue-400 to-indigo-500 flex items-center justify-center text-white font-bold shadow-sm text-lg">
                {selectedAppointment.patientName.charAt(0)}
              </div>
              <div>
                <h2 className="font-bold text-slate-800 tracking-tight">Patient: {selectedAppointment.patientName}</h2>
                <p className="text-xs text-blue-600 font-medium tracking-wide">Assigned to Dr. {selectedAppointment.doctorName}</p>
              </div>
            </div>
          ...
          <div className="px-6 pt-4 pb-2 z-10">
            <div className="rounded-xl bg-blue-50 px-4 py-3 text-sm text-blue-900 shadow-sm border border-blue-100/50">
              <div className="font-semibold mb-1 flex items-center gap-2">
                <Info className="h-4 w-4 text-blue-500" />
                Front Desk Support
              </div>
              <div className="text-blue-800/80">
                You are chatting as <span className="font-semibold text-blue-700">Receptionist</span> for Patient: {selectedAppointment.patientName}. Let them know of any scheduling updates from Dr. {selectedAppointment.doctorName}.
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-2 z-10 space-y-4 flex flex-col">
            {messages.map((m) => {
              const isMe = m.senderRole === "receptionist";
              return (
                <div
                  key={m.id}
        ...
        <div className="flex-1 flex flex-col items-center justify-center bg-[#F8FAFC] relative z-20">
          ...
          <h2 className="text-2xl font-bold text-slate-800 mb-3 tracking-tight">Support Messages</h2>
          <p className="text-sm font-medium text-slate-500 text-center max-w-md leading-relaxed">
            Select a patient from the sidebar to provide front-desk assistance.
          </p>
        </div>
      )}
    </div>
